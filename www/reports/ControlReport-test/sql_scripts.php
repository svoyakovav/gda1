<?php
	...
	//837 = GОП; 164 = G75; 167 = G77; 170 = G78; 516 = 470
	// 13.12.2018 ЕЛТ не берем наименование груза из истории  состояния вагонов из-за задвоения записей (один и тот же вагон попадает в несколько категорий груза)
	// 13.12.2018 ЕЛТ комментим $ProductCategory
...
/*25*/ 			(case when REF020.NAME like '%КАССА%' then 122 else 46 end )
			...
			ELSE 666 END
		";
/*case when REF020.NAME like '%КАССА%' then 45 else 46 end*/
	...
	$RawCategory = "
		...
		";
/*
		case when TO_CHAR(NVL(FIRST_MS002.WEIGHT_FREIGHT, 0), '999G999G990D999') < 10 and $RawCategory ='666' then null else
			'sql_script01/ '||
			CAR.CAR_NUMBER end as CAR_NUMBER,
			
					case when $RawCategory='666' then null else 'sql_script01+/ '||
			LN.CONT_NUMBER end as CONT_NUMBER,	
...
/*18!*/		CASE WHEN (NVL(FIRST_MS002.WEIGHT_FREIGHT, 0) <= 0) THEN 0 ELSE 1 END 
			 AS FWEIGHT_SIGN,
			$Org AS ORGANIZATION_ID,
			$Owner AS CAR_OWNER_SIGN,
			'CAR' AS UNIT_TYPE_CODE, 
			case when (XX_USER.CUSTOMER_PKG.CHECK_PARTY_CURRENT(NVL2(CAR.CARD_HISTORY_ID, CCH.CNT001_OWNER_ID, CAR.CAR_OWNER_ID)) = 1) then 'Куйбышевазот'
                 when (NVL2(CAR.CARD_HISTORY_ID, CCH.CAR002_ID, CAR.CAR_GROUP_ID) = 1) then 'Аренда'
                 else 'Привлеченный' end as car_belong,
...
/*25*/      -- ########## -- 
      LEFT JOIN ASUTL.MS037_CONT_FREIGHT_HISTS MS037 ON MS037.ID = car.FREIGHT_LINE_ID 
      LEFT JOIN ASUTL.REF020_ENTERPRISE_FREIGHT REF020 ON REF020.ID = MS037.REF020_ID 	
		
		UNION ALL
		
		SELECT 
			/*case when $RawCategory='666' then null else*/ 
			/*'sql_script01+/ '||*/
			LN.CONT_NUMBER as CONT_NUMBER,
			
			(  SELECT  
                   (case when inf.Place_Type=4 then 'Площадка [ ' ||inf.REF090_ROW_NUMBER||' / '||inf.REF090_PLACE_STR||' / '|| inf.REF090_NUM_LEVEL || ' ]'
                         when inf.place_type=1 then 'Платформа [ ' ||inf.car_number|| ' ]'
                         when inf.place_type=3 then 'Место хранения [ ' ||inf.HR003_NAME|| ' ]'
                         when inf.place_type=2 then 'Транспортное средство'
                         else 'Неизвестно'  
                   end )
              FROM  ASUTL.MS_CONT_CUR_INFO inf
              WHERE inf.CAR025_ID=LN.CONT_ID
            )
			  as cont_location,
...
/*25*/			nvl(REF020.NAME, 'ЕТСНГ: '||FIRST_NSI008.NAME) AS CARGO_FULL_NAME,
			...
			CASE WHEN (NVL(FIRST_MS031.WEIGHT_FREIGHT, 0) <= 0) THEN 0 ELSE 1 END 
			AS FWEIGHT_SIGN,
...
/*25*/			nvl(REF020.NAME, 'ЕТСНГ: '||FIRST_NSI008.NAME) AS CARGO_FULL_NAME,
			...
			CASE WHEN (NVL(MS003.WEIGHT_FREIGHT, 0) > 0)
				...
				END AS ACTION_TYPE,
			CASE WHEN (NVL(MS003.WEIGHT_FREIGHT, 0) <= 0) THEN 0 ELSE 1 END AS WEIGHT_SIGN,
			CASE WHEN (NVL(FIRST_MS002.WEIGHT_FREIGHT, 0) <= 0) THEN 0 ELSE 1 END 
			AS FWEIGHT_SIGN,
			$Org AS ORGANIZATION_ID,
			$Owner AS CAR_OWNER_SIGN,
			'CAR' AS UNIT_TYPE_CODE,
		...
		SELECT 
			/*case when $RawCategory='666' then LN.CONT_NUMBER else*/
			/*'sql_script03+/ '||*/
			LN.CONT_NUMBER  as CONT_NUMBER,
			(  SELECT  
                   (case when inf.Place_Type=4 then 'Площадка [ ' ||inf.REF090_ROW_NUMBER||' / '||inf.REF090_PLACE_STR||' / '|| inf.REF090_NUM_LEVEL || ' ]'
...
/*25*/			nvl(REF020.NAME, 'ЕТСНГ: '||FIRST_NSI008.NAME) AS CARGO_FULL_NAME,
		...
		SELECT
			/*case when $RawCategory='666' then CONT.CONT_NUMBER else */
			/*'sql_script02+/ '||*/
			...
			(  SELECT  
              ...
              FROM  ASUTL.MS_CONT_CUR_INFO inf
              WHERE inf.CAR025_ID=cont.CONT_ID
            )
			  as cont_location,
			TO_CHAR(MS031.ENDTIME, 'DD.MM.YYYY HH24:MI') AS LOADING_DATE,
...
/*25*/			nvl(REF020.NAME, 'ЕТСНГ: '||FIRST_NSI008.NAME) AS CARGO_FULL_NAME,
			...
			NVL(/*CONT.CARGO_CATEGORY_ID*/case when REF020.NAME like '%КАССА%' then 122 else CONT.CARGO_CATEGORY_ID end, $ProductCategory) as CARGO_CATEGORY_ID,
			$RawCategory AS FCARGO_CATEGORY_ID,
			CONT.UNIT_LENGTH
		FROM (
			SELECT 
