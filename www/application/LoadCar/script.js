...
function runAction(pActionName, pRowId, pAttr1, pAttr2, pAttr3) {
	...
	switch(pActionName) {
		case 'ClickCar':
			...
			//vArray[4] - наличие Закрутки: 1 - да; 0 - нет
			...
			var vOrgTableCurr = $('#P1_ORGANIZATION_FRONT').val();
...
//						 "tdMenuSertificat", "Формировать сертификат качества", "runAction('SertificatShape')", (vArray[1] == '' && vArray[3] == '1') ? '1' : '0',				 
						 ...
						 "tdMenuPrintF3", "Форма №3", "runAction('Form3')", (vArray[0] == '1' && vArray[1] == '' && vArray[2] == '1' && vArray[3] == '1' && vOrgTableCurr=='192') ? '1' : '0', 
						// "tdMenuPrintF3", "Форма №3", "runAction('Form3')", '1', //test alex

						 "tdMenuSprut", "Корректировать ЗПУ", "runAction('SprutCar')", (vArray[1] == '' && vArray[3] == '1' && vArray[4] == '0') ? '1' : '0',
		...
		case 'Form3':
			vData = dbQuery('Form3', gRowId, '', '');
			document.getElementById('Form3').innerHTML = vData;
			openModal('Form3', 'Форма №3' );
			break;		
...
/*elt, добавление расчета п.м.*/
		...
		case 'ChangeMethodDefinition':
			...
			gPrevious = pAttr1;
			$('#P1_REF003_ID').val(0);
			if  (searchOnLine(pAttr1, 'ВЗВЕШ.') == 1 && $('#P1_ORGANIZATION_FRONT').val() == '195' ) {$('#ButGetCargoWeight').prop('disabled',false);} 
				else {$('#ButGetCargoWeight').prop('disabled',true);} 
			break;
			
		//получение данных по взвешиванию вагона
		case 'getCargoWeight':	
			var vOrg_Front_Id = $('#P1_ORGANIZATION_FRONT').val();
			if (confirm('Обновить вес нетто данными по взвешиванию вагона?')) {					
				vData = dbQuery('getCargoWeight', gVar01 + '|' + gRowId + '|' + vOrg_Front_Id ,'','');  //номер вагона, ид вагона, ид организации
				var vArray = vData.split('|'); 
				if (vArray[0] == 0 ) {
					$('div#IngredientTable .modalItem[name="f01"]:eq(0)').val(vArray[3]);					
					$('div#IngredientTable .modalItem[name="f01"]:eq(0)').prop('disabled',true);
					switch (vOrg_Front_Id) {
					case '195': $('#P1_REF003_ID').val(36); break;
					default:    $('#P1_REF003_ID').val(0);}}
		...
		case 'CreateLot':
			...
			if (confirm(vMessage)) {
				var vLineValue = gRowId + '|' + gVar01 + '|' + gVar03 + '|' + pAttr1 + '|' + $('#P1_REF003_ID').val();
				//alert(vLineValue);
		...
		case 'SprutCar':
			...
			for (var i = 1; i < vArray.length; i++) {
				...
				if ($('#P1_SPRUT_TYPE option:selected').text().toUpperCase()=='ЗАКРУТКА') {		
					$s('P1_SPRUT_S' + i, '');
					$s('P1_SPRUT_N' + i, vArray[i].substr(1, 7));}
				else {$s('P1_SPRUT_S' + i, vArray[i].substr(3, 1));
					  $s('P1_SPRUT_N' + i, vArray[i].substr(4));}
			  }
...
function ExportExcel(TableId, FileName)
 {
  ...
  var tab_cell_text;
  var tab_text = '<table><tr><td align="center" colspan="2"> <h3>Форма №3<br>Сдаточный акт на заполненную аммиаком цистерну</h3></td></tr><tr><td></td></tr></table>';
  tab_text = tab_text + "<table border='1px'>";
  for(var j = 1 ; j < tab.rows.length ; j++)
   {
 	switch (j) {
 case 2:
	//tab_text = tab_text + '<tr><td>Регистрационный номер цистерны</td><td align="right">' + $('#P1_FORM3_REG_NUM_CAR').val()+ '</td></tr>';
    break;
  case 3:
  	//tab_text = tab_text + '<tr><td>Дата наполнения цистерны аммиаком</td><td align="right">'+ $('#P1_FORM3_DATE_LOAD').val()+ '</td></tr>';
    break;
  case 4:
	tab_text = tab_text + '<tr><td>Давление в цистерне после наполнения</td><td align="right">'+ $('#P1_FORM_CAR_PRESSURE').val() + '</td></tr>';
    break;
  default:
 ...
 // tab_text = tab_text + '<br><p>Подписи:</p> <p>Начальник смены _____________<br><br> Сливщик-разливщик ___________</p>';
...
function ExportExcel_Rep(TableId)
 {
  tab = $('#report_ReportHistory table')[0];
  var checks = document.getElementsByName('f01');
  var tab_cell_text;
  var tab_text='';
  var tab_text_header = '<table><tr><td align="center" colspan="2"> <h3>Форма №3<br>Сдаточный акт на заполненную аммиаком цистерну</h3></td></tr><tr><td></td></tr></table>';
  
  for(var j = 1 ; j < tab.rows.length ; j++)
   {
	if (checks[j-1].checked) {      
	tab_text = tab_text+tab_text_header+"<table border='1px'>";
	tab_text = tab_text + '<tr><td>Железнодорожный номер цистерны</td><td align="right">' + tab.rows[j].cells[0].innerText+ '</td></tr>';
	tab_text = tab_text + '<tr><td>Давление в цистерне после наполнения</td><td align="right"> 7 кг/см2</td></tr>';
	tab_text = tab_text + '<tr><td>Наполнение цистерны аммиаком и ее осмотр перед отправкой потребителю произвел</td><td align="right"> ' + tab.rows[j].cells[13].innerText+ '</td></tr>';
	tab_text = tab_text + '<tr><td>Номер ЗПУ</td><td align="right"> ' + tab.rows[j].cells[11].innerText+ '</td></tr>';
	tab_text = tab_text + '<tr><td>Дата</td><td align="right"> ' + tab.rows[j].cells[3].innerText+ '</td></tr>';
  ...
  if (msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./)) //If Internet Explorer
   {
    ...
    txtArea1.document.close();
    txtArea1.focus(); 
    sa = txtArea1.document.execCommand("SaveAs", true, 'report_f3.xls');	
   }  
  else //other browser not tested on IE 11
   {
    sa = window.open('data:application/vnd.ms-excel,' + encodeURIComponent(tab_text));
