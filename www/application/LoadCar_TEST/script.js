	...
	switch(pActionName) {
		case 'ClickCar':
			...
			var vMenu = ["tdMenuData", "Информация по вагону", "runAction('DataCar')", (vArray[0] == '1' /*&& vArray[1] == ''*/) ? '1' : '0',
						 ...
						 "tdMenuClean", "Убрать вагон", "runAction('CleanCar', '', 'Подтверждаете уборку подвижной единицы')", (vArray[0] == '1' && vArray[1] == '') ? '1' : '0',
...
/*29-01-2019*/				
		case 'SertificatPrint':
			vData = dbQuery(pActionName, '', '', '', 1);
			//alert('SertificatPrint is null');
...
function dbQuery(pProcessName, pLineValue, pProcedureName, pItemId, pArrayQty) {
	...
	if (pProcessName != '') {
		...
		} else if (pProcessName == 'SertificatPrint'){
			if (pArrayQty != undefined) {
				for (var i = 1; i <= pArrayQty; i++) {
					vRequest.AddArrayItems(apex.jQuery('[name=f' + ((i <= 9) ? '0' + i : i) + ']'), i);
				}
			}
		}
